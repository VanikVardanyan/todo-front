...
import { ITaskItemProps } from "../types";
  ...
  export const TaskItem: React.FC<ITaskItemProps> = ({ task, handleCompleteTask }) => (
    <div className="mt-4 border p-4 rounded-md bg-white">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          onClick={task.completed ? undefined : () => handleCompleteTask(task._id)}
          disabled={task.completed}
        >
          {task.completed ? 'Готово' : 'В архив'}
        </button>
      </div>
      <p className={`mt-2 ${task.completed ? 'line-through text-gray-500' : 'text-black'}`}>
        {task.description}
      </p>
    </div>
  );
